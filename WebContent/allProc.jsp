	...
	try{
		...
		String read=""; //파일을 읽어올 변수
		 while((read= reader.readLine())!=null){ //파일의 끝까지 반복
			...
			String[] track = str[3].split("/");//파일에서 가져온 트랙리스트를  /를 기준으로 추출
...
//			out.println(tmp+","+albumArt(tmp)+"<br>");
			...
			if(imgatr!=null){ //이유는 모르겠지만..첫번째 데이터가 null값이 넘겨져서... null이 아닐때만 tr생성..하는걸로..				
				...
				<td><img class="image" src="img/<%=imgatr%>"></td>
				<td><a target="black" href="https://www.melon.com/album/detail.htm?albumId=<%=albumId%>"><%=song.get(i).Album%></a></td>
